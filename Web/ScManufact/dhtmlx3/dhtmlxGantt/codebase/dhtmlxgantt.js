// v.1.3 build 100805
/*
Copyright (C) 1998-2012 DHTMLX LTD. http://www.dhtmlx.com
 ...
 *  @topic: 0
 */
function GanttProjectInfo(id, name, startDate,planH)
{
    this.Id = id;
    this.Name = name;
...
{
    if (!this.StartDate || this.StartDate > task.StartDate)
 ...
 * @topic: 0
 */
function GanttTaskInfo(id, name, startDate, duration, percentCompleted, predecessorTaskId, planH, alarm)
{
    this.Id = id;
    this.Name = name;
    this.StartDate = startDate;
...
function GanttChart()
{
    ...
    this.shortMonthNames = ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"];
    this.monthNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    this._useShortMonthNames = true;

	
	//show or hide menu items
	this.menuTaskCode="111111111";
	this.menuProjectCode="111111";
	...
	this.maxId = 0;
	//lang
	this.ChartLang = new LangName();
    dhtmlxEventable(this);
}

GanttChart.prototype.setSelHandler = function(fn)
{
    this.selHandler = fn;
};

GanttChart.prototype.setMenuTaskCode = function(code)
{
    this.menuTaskCode = code;
};
GanttChart.prototype.setMenuProjectCode = function(code)
{
    this.menuProjectCode = code;
};
    ...
    if (project)
    {
        ...
        if (this.Project.length == 0)
        {
            if (this.isShowNewProject)
            {
                ...
                var pi = new GanttProjectInfo(1, "New project", d,0);
 ...
 * @topic: 1
 */
GanttProject.prototype.insertTask = function(id, name, StartDate, Duration, PercentCompleted, predecessorTaskId, parentTaskId, planH, alarm)
{
    var task = null;
    var _task = null;
        ...
        if (predecessorTaskId != "")
        {
            ...
            if (predTask.ParentTask) {
                if (predTask.ParentTask.Id != task.ParentTask.Id) {
                    this.Chart.Error.throwError("DATA_INSERT_ERROR", 32, [predTask.Id+":"+predTask.Name,task.Id+":"+task.Name]);
                    return false;
                }
            } else {
                this.Chart.Error.throwError("DATA_INSERT_ERROR", 32, [predTask.Id+":"+predTask.Name,task.Id+":"+task.Name]);
    ...
    } else
    {
        ...
        if (task.StartDate <= this.Chart.startDate) {
            this.Chart.Error.throwError("DATA_INSERT_ERROR", 18, [task.Id+":"+task.Name]);
            return false;
        }
        ...
        if (predecessorTaskId != "") {
            ...
            if (predTask.ParentTask) {
                this.Chart.Error.throwError("DATA_INSERT_ERROR", 15, [task.Id+":"+task.Name,predTask.Id+":"+predTask.Name]);
                return false;
            }
            task.PredecessorTask = predTask;
        }
    ...
    for (var t = 0; t < parentTask.ChildTasks.length; t++)
    {
        ...
        if (!this.checkPosParentTask(parentTask, parentTask.ChildTasks[t]))
        {
            if (!this.correctError) {
                ...
                this.Error.throwError("DATA_ERROR", 28, [parentTask.Id+":"+parentTask.Name,parentTask.ChildTasks[t].Id+":"+parentTask.ChildTasks[t].Name]);
                return true;
            } else {
        ...
        {
            if (!this.correctError) {
                this.Error.throwError("DATA_ERROR", 33, [parentTask.Id+":"+parentTask.Name,parentTask.ChildTasks[t].Id+":"+parentTask.ChildTasks[t].Name]);
                return true;
            } else {
                this.correctPosParentTask(parentTask, parentTask.ChildTasks[t]);
            }
    ...
    for (var k = 0; k < project.ParentTasks.length; k++) {
        ...
        if (project.ParentTasks[k].PredecessorTask)
        {
            if (!this.checkPosPredecessorTask(project.ParentTasks[k].PredecessorTask, project.ParentTasks[k])) {
                if (!this.correctError) {
                    this.Error.throwError("DATA_ERROR", 26, [project.ParentTasks[k].PredecessorTask.Id+":"+project.ParentTasks[k].PredecessorTask.Name,project.ParentTasks[k].Id+":"+project.ParentTasks[k].Name]);
                    return true;
                } else {
...
GanttChart.prototype.setPredTaskInTree = function(parentTask)
{
    ...
    for (var t = 0; t < parentTask.ChildTasks.length; t++)
    {
        if (!this.isEmpty(parentTask.ChildTasks[t].PredecessorTaskId))
        {
            ...
            {
                if (!this.correctError) {
                    this.Error.throwError("DATA_ERROR", 26, [parentTask.ChildTasks[t].PredecessorTask.Id+":"+parentTask.ChildTasks[t].PredecessorTask.Name,parentTask.ChildTasks[t].Id+":"+parentTask.ChildTasks[t].Name]);
                    return true;
                } else {
 ...
 * @topic: 1
 */
GanttChart.prototype.insertProject = function(id, name, startDate,planH)
{
    if (this._isError)
    {
        this.clearData();
    ...
    if (this.getProjectById(id)) {
        this.Error.throwError("DATA_INSERT_ERROR", 23, [id+":"+name]);
        return false;
    }

    this.checkHeighPanelTasks();

    var project = new GanttProjectInfo(id, name, startDate,planH);
...
GanttChart.prototype._showContextMenu = function(event, obj)
{
    ...
    if (this.contextMenu.isDhtmlxMenuObject) {
		...
		x=parseInt(elem.style.left)+parseInt(elem.offsetWidth);
		y=parseInt(elem.style.top)+parseInt(elem.offsetHeight);
        this.contextMenu.showContextMenu(x, y, obj);
    }
...
GanttTask.prototype.setPredecessor = function(predecessorTaskId)
{
    ...
    else
    {
		...
		{
			if (parseInt(task.Duration) + parseInt(predTask.Duration) > this.TaskInfo.ParentTask.Duration)
			{
				this.Chart.Error.throwError("DATA_INSERT_ERROR", 19, [this.TaskInfo.ParentTask.Id+":"+this.TaskInfo.ParentTask.Name,task.Id+":"+task.Name]);
				return false;
			}
		}
...
GanttTask.prototype.setStartDate = function(startDate, shiftChild)
{
    ...
    this.getMoveInfo(pos);

	/*
    if ((parseInt(this.cTaskItem[0].firstChild.firstChild.width) + pos > this.maxPosXMove) && (this.maxPosXMove != -1))
    {
        this.Chart.Error.throwError("DATA_INSERT_ERROR", 12, [this.TaskInfo.Id+":"+this.TaskInfo.Name]);
        this.maxPosXMove = -1;
        this.minPosXMove = -1;
        return false;
    }
	*/
    ...
    {
        this.Chart.Error.throwError("DATA_INSERT_ERROR", 11, [this.TaskInfo.Id+":"+this.TaskInfo.Name]);
        this.maxPosXMove = -1;
        this.minPosXMove = -1;
        return false;
    }
...
GanttTask.prototype.setTaskplanH = function(planH)
{
    this.getResizeInfo();
	this.TaskInfo.planH = planH;	
    var width = this.Chart.getWidthOnDuration(this.TaskInfo.Duration);
...
GanttTask.prototype.setTaskAlarm = function(alarm)
{
	this.TaskInfo.alarm = alarm;	
	this.resizeTaskAlarm();
	if (this.Chart.isShowDescTask)this.descrTask.innerHTML = this.getDescStr();
	return true;
};
...
GanttTask.prototype.setDuration = function(duration)
{
    ...
    this.getResizeInfo(width);
    if ((width > this.maxWidthResize) && (this.maxWidthResize != -1))
    {

        //this.Chart.Error.throwError("DATA_INSERT_ERROR", 10, [this.TaskInfo.Id+":"+this.TaskInfo.Name]);
        //return false;
    } else if (width < this.minWidthResize)
    {
        this.Chart.Error.throwError("DATA_INSERT_ERROR", 9, [this.TaskInfo.Id+":"+this.TaskInfo.Name]);
        return false;
    } 
	...
	if (day > 0)
	{
		...
		{
			startdate = this.childPredTask[i].TaskInfo.StartDate - day * 24 * 60 * 60 * 1000;
	...
	{
		var maxFinishDate=0;
		for (i=0; i< this.parentTask.childTask.length; i++)
		{
			if (this.parentTask.childTask[i].getFinishDate() > maxFinishDate)
			{
				maxFinishDate = this.parentTask.childTask[i].getFinishDate();
			}
		}
		if (this.parentTask.getFinishDate() > maxFinishDate)
		{
			var dur = (this.parentTask.getFinishDate() - maxFinishDate) / (24*60*60*1000) * this.Chart.hoursInDay;
			dur = parseInt(this.parentTask.TaskInfo.Duration) - parseInt(dur) ;
			this.parentTask.setDuration(dur);
		}
	}
	this.resizeTaskAlarm();
	return true;
    ...
    if ((name != "") && (name != null)) {
        ...
        if (this.Chart._showTreePanel)
        {
            ...
            this.cTaskNameItem[0].title = name;
			 this.checkWidthTaskNameItem(); //标记1
        }
        if (this.Chart.isShowDescTask)this.descrTask.innerHTML = this.getDescStr();
        this.addDayInPanelTime();
    ...
    for (var i = 0; i < this.Project.length; i++)
    {
        for (var k = 0; k < this.Project[i].ParentTasks.length; k++)
        {
            if ((this.Project[i].ParentTasks[k].StartDate != null) && (this.Project[i].ParentTasks[k].StartDate != '')) {
                ...
                this.Error.throwError("DATA_ERROR", 25, [this.Project[i].ParentTasks[k].Id+":"+this.Project[i].ParentTasks[k].Name]);
                return;
            }
        ...
        for (var k = 0; k < this.Project[i].ParentTasks.length; k++) {
            if (this.Project[i].ParentTasks[k].StartDate < this.Project[i].StartDate) {
                this.Error.throwError("DATA_ERROR", 24, [this.Project[i].ParentTasks[k].Id+":"+this.Project[i].ParentTasks[k].Name,this.Project[i].Id+":"+this.Project[i].Name]);
                return;
            }
...
GanttChart.prototype.doLoadDetails = function(isLocal)
{
    ...
    for (var j = 0; j < projectArr.length; j++)
    {
        ...
        var project = new GanttProjectInfo(projectArr[j].getAttribute("id"), projectArr[j].getAttribute("name"), new Date(startDate[0], (parseInt(startDate[1]) - 1), startDate[2]),projectArr[j].getAttribute("planH"));
        ...
        for (var i = 0; i < taskArr.length; i++) {
			...
			if (parseInt(id) > parseInt(this.maxId))
			{
				this.maxId = id;
			}
			...
			planH = (this.xmlLoader.doXPath("./planH", taskArr[i])[0].firstChild == null) ? "" : this.xmlLoader.doXPath("./planH", taskArr[i])[0].firstChild.nodeValue;

			alarm = (this.xmlLoader.doXPath("./alarm", taskArr[i])[0].firstChild == null) ? "" : this.xmlLoader.doXPath("./alarm", taskArr[i])[0].firstChild.nodeValue;
            ...
            var task = new GanttTaskInfo(id, name, new Date(startDate[0], (parseInt(startDate[1]) - 1), startDate[2]), duration, percentCompleted, predecessorTaskId, planH, alarm);
...
GanttChart.prototype.readChildTasksXML = function(parentTask, childTasksArrXML)
{
    ...
    for (var i = 0; i < childTasksArrXML.length; i ++)
    {
        ...
        planH = (this.xmlLoader.doXPath("./planH", childTasksArrXML[i])[0].firstChild == null) ? "" : this.xmlLoader.doXPath("./planH", childTasksArrXML[i])[0].firstChild.nodeValue;

		alarm = (this.xmlLoader.doXPath("./alarm", childTasksArrXML[i])[0].firstChild == null) ? "" : this.xmlLoader.doXPath("./alarm", childTasksArrXML[i])[0].firstChild.nodeValue;
...
/**
 * @desc: create XML string from the chart content   返回XML文档
 * @type: public
 * @topic: 6
 */
GanttChart.prototype.getXML = function()
{
    ...
    for (var i = 0; i < this.Project.length; i++)
    {
        ...
        strXML += "<project id ='" + this.Project[i].Id + "' name= '" + this.Project[i].Name + "' planH = '"+this.Project[i].planH+"' startdate = '" + this.Project[i].StartDate.getFullYear() + "," + (this.Project[i].StartDate.getMonth() + 1) + "," + this.Project[i].StartDate.getDate() + "'>";
        ...
        {		
            ...
            strXML += "<planH>" + this.Project[i].ParentTasks[j].planH + "</planH>";
			strXML += "<alarm>" + this.Project[i].ParentTasks[j].alarm + "</alarm>";
...
GanttChart.prototype.createChildTasksXML = function(childTasks)
{
    ...
    for (var n = 0; n < childTasks.length; n++)
    {
        ...
        strXML += "<planH>" + childTasks[n].planH + "</planH>";
		strXML += "<alarm>" + childTasks[n].alarm + "</alarm>";
		...
		if(childTasks[n].PredecessorTaskId == null)  childTasks[n].PredecessorTaskId = "";
...
GanttChart.prototype.getMonthScaleLabel = function(date) {
    return (this._useShortMonthNames ? this.shortMonthNames : this.monthNames)[date.getMonth()] + " '" + (""+date.getFullYear());
};
/**
...
GanttTask.prototype.getPopUpInfo = function(object, event)
{
    ...
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>ID:&nbsp;</span><span class='ut'>" + this.TaskInfo.Id + "</span><br/>";


    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.tStartDate +":&nbsp;</span><span class='ut'>" + this.TaskInfo.StartDate.getFullYear() + "-" + (this.TaskInfo.StartDate.getMonth() + 1) + "-" + this.TaskInfo.StartDate.getDate() + "</span><br/>";

	var propValue = this.getFinishDate();
    var str = propValue.getFullYear() + "-" + (propValue.getMonth() + 1) + "-" + propValue.getDate();
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.tEndDate +":&nbsp;</span><span class='ut'>" + str + "</span><br/>";

	 

    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.tDuration +":&nbsp;</span><span class='ut'>" + this.TaskInfo.Duration + "</span><br/>";
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st' style='color:blue'>" + lang.tPlanH +":&nbsp;</span><span class='ut' style='color:blue'>" + this.TaskInfo.planH + "</span><br/>";

    tblInfo.rows[0].cells[0].innerHTML += "<span class='st' style='color:blue'>" + lang.tAlarm +":&nbsp;</span><span class='ut' style='color:blue'>" + this.TaskInfo.alarm + "</span><br/>";

    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.tPercent +":&nbsp;</span><span class='ut'>" + this.TaskInfo.PercentCompleted + "</span><br/>";

    //show predecessor task
    if (this.predTask)
    {
        tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.tPredecessorTask +":&nbsp;</span>";
    ...
    if (this.TaskInfo.ChildTasks.length != 0) {
        tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.tChildTask +":&nbsp;</span>";
    ...
    //show parent task
    if (this.TaskInfo.ParentTask) {
        tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.tParentTask +":&nbsp;</span>";
...
GanttTask.prototype.createConnectingLinesDS = function()
{
    ...
    {
        lineVerticalRight.style.cssText = "border-width: 0px 0px 0px 1px; border-style: solid; border-color: #4A8F43;margin: 0px; padding: 0px;z-index:1;font-size: 1px;position: absolute;" +
        ...
        lineHorizontal.style.cssText = "height:1px;border-color: #4A8F43;border-style: solid;border-width: 1px 0px 0px 0px;margin: 0px; padding: 0px;z-index:1;position: absolute;" +
        ...
        arrowImg.style.cssText = "z-index:1;margin: 0px; padding: 0px;width:7px;height:14px;position: absolute;left:" + (posXChildTask - 7) + "px;top:" + (posYChildTask - 1) + "px;";
    } else {
        lineVerticalRight.style.cssText = "border-width: 0px 0px 0px 1px; border-style: solid; border-color: #4A8F43;margin: 0px; padding: 0px;z-index:1;font-size: 1px;position: absolute;" +
...
GanttProject.prototype.getPopUpInfo = function(object, event)
{
    ...
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.pStartDate +":&nbsp;</span><span class='ut'>" + this.Project.StartDate.getFullYear() + "-" + (this.Project.StartDate.getMonth() + 1) + "-" + this.Project.StartDate.getDate() + "</span><br/>";

	var propValue = this.getFinishDate();
    var str = propValue.getFullYear() + "-" + (propValue.getMonth() + 1) + "-" + propValue.getDate();
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.pEndDate +":&nbsp;</span><span class='ut'>" + str + "</span><br/>";



    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.pDuration +":&nbsp;</span><span class='ut'>" + this.Duration + "</span><br/>";
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st' style='color:blue'>" + lang.pPlanH +":&nbsp;</span><span class='ut' style='color:blue'>" + this.Project.planH + "</span><br/>";

    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>" + lang.pPercent +":&nbsp;</span><span class='ut'>" + this.percentCompleted + "</span><br/>";
...
GanttProject.prototype.resizeProjectItem = function(width)
{
	...
	this.resizeProjectPlan(countHours);
};

GanttProject.prototype.resizeProjectPlan = function(hours)
{
	var divPlan=document.getElementById("prodiv_" + this.Project.Id)
	var imgPlan=document.getElementById("proimg_" + this.Project.Id)
	
	if (parseInt(this.Project.planH) > parseInt(hours))
	{	
		divPlan.style.left=hours * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.width = (this.Project.planH - hours) * this.Chart.hourInPixelsWork + "px";
		imgPlan.src = this.Chart.imgs + "progress_less.png";
		
	}
	if (parseInt(this.Project.planH) < parseInt(hours))
	{	
		divPlan.style.left=this.Project.planH * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.width = (hours-this.Project.planH) * this.Chart.hourInPixelsWork + "px";
		imgPlan.src = this.Chart.imgs + "progress_more.png";
	}
	if (parseInt(this.Project.planH) == parseInt(hours))
	{	
		divPlan.style.left=this.Project.planH * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.width ="0px";
	}
...
GanttTask.prototype.resizeTaskItem = function(width)
{
	...
	this.resizeTaskPlan(countHours);
	
	
};
GanttTask.prototype.resizeTaskAlarm = function()
{
	if (parseInt(this.TaskInfo.alarm) >= 0)
	{
		...
		var alarmDate = addDate(sd, parseInt(this.TaskInfo.Duration / this.Chart.hoursInDay -this.TaskInfo.alarm));
		var divAlarm=document.getElementById("divAlarm_" + this.TaskInfo.Id);
		var imgAlarm=document.getElementById("imgAlarm_" + this.TaskInfo.Id);

		if (now >= alarmDate && now <= ed)
		{
			divAlarm.style.left = this.TaskInfo.Duration * this.Chart.hourInPixelsWork - this.TaskInfo.alarm * this.Chart.hoursInDay * this.Chart.hourInPixelsWork + "px";
			imgAlarm.style.width = this.TaskInfo.alarm * this.Chart.hoursInDay * this.Chart.hourInPixelsWork + "px";
		}
		else
		{
			divAlarm.style.left = "0px";
			imgAlarm.style.width = "0px";
		}
	}
}

GanttTask.prototype.resizeTaskPlan = function(hours)
{
	var divPlan=document.getElementById("divplan_" + this.TaskInfo.Id);
	var imgPlan=document.getElementById("plan_" + this.TaskInfo.Id);

	if (parseInt(this.TaskInfo.planH) > parseInt(hours))
	{	
		divPlan.style.left=hours * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.width = (this.TaskInfo.planH - hours) * this.Chart.hourInPixelsWork + "px";
		imgPlan.src = this.Chart.imgs + "progress_less.png";
		
	}
	if (parseInt(this.TaskInfo.planH) < parseInt(hours))
	{	
		divPlan.style.left=this.TaskInfo.planH * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.width = (hours-this.TaskInfo.planH) * this.Chart.hourInPixelsWork + "px";
		imgPlan.src = this.Chart.imgs + "progress_more.png";
	}
	if (parseInt(this.TaskInfo.planH) == parseInt(hours))
	{	
...
GanttProject.prototype.moveDescrProject = function()
{
	...
	if (parseInt(this.Duration) < parseInt(this.Project.planH))
	{
		posX = posX + (this.Project.planH - this.Duration) * this.Chart.hourInPixelsWork;
	}
...
GanttProject.prototype.createProjectItem = function()
{
	...
	div_bg.style.left = 0 - this.posX + "px";
	div_bg.style.width = document.getElementById("panelTime").scrollWidth -24 + "px" ;
	div_bg.style.top  = "-6px";

    div_bg.onmouseover = function() {
		if (self.Chart.selProject == null  || self.Chart.selProject != self)
			this.style.background = "url(" + self.Chart.imgs + "bg_hover.png)";
    };
    div_bg.onmouseout = function() {
		if (self.Chart.selProject != self)
	        this.style.background = "";
    };

    div_bg.onclick = function() {
		if (self.Chart.selProject == self){
			self.Chart.selType = null;
			self.Chart.selProject = null;
	        this.style.background = "";
		}
		else{
			...
			self.Chart.selProject = self;
	        this.style.background = "url(" + self.Chart.imgs + "bg_select.png)";
		}
		if (self.Chart.selHandler){
			self.Chart.selHandler();
		}

    };

	projectItem.appendChild(div_bg);

	if (parseInt(this.Project.planH) >= parseInt(this.Duration))
	{
		var divplan = document.createElement("div");
		divplan.id="prodiv_"+this.Project.Id;
		divplan.style.cssText = "z-index:1;position: absolute;left:"+ (this.Duration * this.Chart.hourInPixelsWork) +"px;top:0px;border: 1px solid rgb(101, 137, 169);";
		var divplanH = document.createElement("div");

		var imgPlan = document.createElement("img");
		imgPlan.id="proimg_"+this.Project.Id;
		imgPlan.style.width = (this.Project.planH - this.Duration) * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.height = this.Chart.heightTaskItem + "px";
		divplan.appendChild(imgPlan);
		imgPlan.src = this.Chart.imgs + "progress_less.png";
		
		projectItem.appendChild(divplan);
	...
	if (parseInt(this.Project.planH) < parseInt(this.Duration))
	{
		var divplan = document.createElement("div");
		divplan.id="prodiv_"+this.Project.Id;
		divplan.style.cssText = "z-index:1;position: absolute;left:"+ (this.Project.planH * this.Chart.hourInPixelsWork) +"px;top:0px;border: 1px solid rgb(101, 137, 169);";
		var imgPlan = document.createElement("img");
		imgPlan.id="proimg_"+this.Project.Id;
		imgPlan.style.width = (this.Duration - this.Project.planH) * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.height = this.Chart.heightTaskItem + "px";
		divplan.appendChild(imgPlan);
		imgPlan.src = this.Chart.imgs + "progress_more.png";
		projectItem.appendChild(divplan);
...
GanttProject.prototype.toggleAllTask = function()
{
	var self = this;
	var task = null;
	...
	{
		task=this.getTaskById(this.Project.ParentTasks[i].Id);
		if (this.isOpen && task._isOpen==false)
			task.toggleTree();
		if (this.isOpen==false && task._isOpen==true)
			task.toggleTree();
		task.toggleTree();
	}
	/*
	if (this.isOpen)
	{
		img.src = this.Chart.imgs + "plus.gif";
		this.isOpen=false;
	}
	else
	{
	   //alert(this.Chart.imgs);
		img.src = this.Chart.imgs + "minus.gif";
		this.isOpen=true;
	}
	*/
}
...
/**
 * @desc: Creation of projectNameItem   建立项目菜单
 * @type: private
 * @topic: 0
 */
GanttProject.prototype.createProjectNameItem = function()
{
    ...
    divName.innerHTML = "<img id='proimg' src='"+this.Chart.imgs + "minus.gif' style='cursor:pointer'></img>"+this.Project.Name;
	*/
	...
	divName.onclick=function(){self.toggleAllTask()};
...
GanttTask.prototype.showDescTask = function()
{
	...
	if (parseInt(this.TaskInfo.Duration) < parseInt(this.TaskInfo.planH))
	{
		posX = posX + (this.TaskInfo.planH - this.TaskInfo.Duration) * this.Chart.hourInPixelsWork;
	}
 ...
 * @topic: 4
 */
GanttTask.prototype.getMoveInfo = function(pos)
{
    ...
    } else
    {
        ...
        {
			//		
			if ((pos + widthTaskItem) > (posParentTaskItem + widthParentTaskItem))
			{
				var dur = parseInt((pos - this.posX) / this.Chart.hourInPixelsWork) + parseInt(this.parentTask.TaskInfo.Duration);
				this.parentTask.setDuration(dur); 
			}
			//
        ...
        if ((this.parentTask) && (this.childPredTask.length > 0))
        {
			...
			//		
			if ((pos + widthTaskItem) > this.maxPosXMove)
			{
				var dur = parseInt((pos - this.posX) / this.Chart.hourInPixelsWork) + parseInt(this.parentTask.TaskInfo.Duration);
				this.parentTask.setDuration(dur); 
			}
 ...
 * @topic: 4
 */
GanttTask.prototype.getResizeInfo = function(width)
{
    var taskItem = this.cTaskItem[0];
    var posParentTaskItem = (this.parentTask == null) ? 0 : parseInt(this.parentTask.cTaskItem[0].style.left);
    var widthParentTaskItem = (this.parentTask == null) ? 0 : parseInt(this.parentTask.cTaskItem[0].childNodes[0].firstChild.width);
	...
	this.minWidthResize = this.Chart.hourInPixelsWork * this.Chart.minWorkLength;
    ...
    if ((this.childPredTask.length > 0) && (!this.parentTask))
    {
		...
		if (width > this.maxWidthResize)
		{
			var startdate = this.getDateOnPosition(posTaskItem + width);
			this.childPredTask[0].setStartDate(startdate,true);
		}
    ...
    } else if ((this.childPredTask.length > 0) && (this.parentTask))
    {
		...
		if (width > w1)
		{
			var dur=(width - this.Chart.getWidthOnDuration(this.TaskInfo.Duration)) / this.Chart.hourInPixelsWork;
			dur = parseInt(dur) + parseInt(this.parentTask.TaskInfo.Duration);
			this.parentTask.setDuration(dur);
		}
...
GanttTask.prototype.createTaskItem = function()
{
    ...
    divTaskName.style.cssText = ";z-index:3;position: absolute;left:0px;top:0px;";
	...
	divAlarm.style.cssText = "z-index:2;position: absolute;top:0px;border: 1px solid rgb(101, 137, 169);";
	divAlarm.style.left = "0px";

	var imgAlarm = document.createElement("img");
	imgAlarm.id="imgAlarm_"+this.TaskInfo.Id;
	imgAlarm.style.height = this.Chart.heightTaskItem + "px";
	divAlarm.appendChild(imgAlarm);
	imgAlarm.src = this.Chart.imgs + "alarm.png";
	imgAlarm.style.width = "0px";
	if (parseInt(this.TaskInfo.alarm) >= 0)
	{
		//now() later than Alarm day and now() less than finish day 
	...
	itemControl.appendChild(divAlarm);

	if (parseInt(this.TaskInfo.planH) >= parseInt(this.TaskInfo.Duration))
	{
		var divplan = document.createElement("div");
		divplan.id="divplan_"+this.TaskInfo.Id;
		divplan.style.cssText = "z-index:1;position: absolute;left:"+ (this.TaskInfo.Duration * this.Chart.hourInPixelsWork) +"px;top:0px;border: 1px solid rgb(101, 137, 169);";
		//var divplanH = document.createElement("div");

		var imgPlan = document.createElement("img");
		imgPlan.id="plan_"+this.TaskInfo.Id;
		imgPlan.style.width = (this.TaskInfo.planH - this.TaskInfo.Duration) * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.height = this.Chart.heightTaskItem + "px";
		divplan.appendChild(imgPlan);
		imgPlan.src = this.Chart.imgs + "progress_less.png";
		
		itemControl.appendChild(divplan);
	}
	...
	if (parseInt(this.TaskInfo.planH) < parseInt(this.TaskInfo.Duration))
	{
		var divplan = document.createElement("div");
		divplan.id="divplan_"+this.TaskInfo.Id;
		divplan.style.cssText = "z-index:1;position: absolute;left:"+ (this.TaskInfo.planH * this.Chart.hourInPixelsWork) +"px;top:0px;border: 1px solid rgb(101, 137, 169);";
		var imgPlan = document.createElement("img");
		imgPlan.id="plan_"+this.TaskInfo.Id;
		imgPlan.style.width = (this.TaskInfo.Duration - this.TaskInfo.planH) * this.Chart.hourInPixelsWork + "px";
		imgPlan.style.height = this.Chart.heightTaskItem + "px";
		divplan.appendChild(imgPlan);
		imgPlan.src = this.Chart.imgs + "progress_more.png";
		itemControl.appendChild(divplan);
	...
	div_bg.style.left = 0 - this.Chart.getPosOnDate(this.TaskInfo.StartDate) + "px";
	div_bg.style.width = document.getElementById("panelTime").scrollWidth -24 + "px" ;
	div_bg.style.top  = "-6px";

    div_bg.onmouseover = function() {
		if (self.Chart.selTask == null  || self.Chart.selTask != self)
			this.style.background = "url(" + self.Chart.imgs + "bg_hover.png)";
    };
    div_bg.onmouseout = function() {
		if (self.Chart.selTask != self)
	        this.style.background = "";
    };
	...
	itemControl.appendChild(div_bg);
...
GanttTask.prototype.createTaskNameItem = function(hasChildren)
{
    var self = this;
    var divName = document.createElement("div");
    divName.id = "div_"+this.TaskInfo.Id;
	...
	divName.onclick=function(){self.toggleTree()};
    if (this.Chart.isShowConMenu)
    {
        var showContMenu = function(event) {
...
/**
 * @desc: show or hide the node use in GanttTask.prototype.createTaskNameItem and GanttTask.prototype.createTreeImg
 * @type: private
 * @topic: 4
 */

GanttTask.prototype.toggleTree = function()
{
	var self=this;
...
GanttTask.prototype.createTreeImg = function()
{
    ...
    treeImg.id = "nodeimg_" + this.TaskInfo.Id;

    treeImg.onclick = function(){self.toggleTree()};
...
dhtmlXMLSenderObject.prototype.sendData = function(filename, path, xmlData)
{
    ...
    //this.xmlHttp.onreadystatechange=returnstate(); 
	*/
	
	$.ajax({
	url: this.path,
	type: 'POST',
	data:{'data':encodeURI(xmlData),'filename':filename},
	dataType: 'text',
...
contextMenu.prototype._init = function()
{
    ...
    var tab1 = this.createTab(1,1, lang.tRename, "t", true, this);
    tab1.addItem(1, lang.newName, document.createElement("input"), "text", function() {
        tab1.arrItems[0].control.focus();
    });
    tab1.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
    ...
    var tab2 = this.createTab(2,2, lang.tDelete, "t", true, this);

    tab2.addItem(1, lang.buttonOk, document.createElement("input"), "button",
            function()
            {
                try {
    ...
    var tab3 = this.createTab(3,3, lang.labSet+lang.tStartDate, "t", true, this);
    tab3.addItem(1, lang.tStartDate, document.createElement("input"), "calendar", function() {
        tab3.arrItems[0].control.focus();
		var d = tab3.object.TaskInfo.StartDate;
		this.value=d.getFullYear() + "-" + ((d.getMonth() + 1) > 9 ? (d.getMonth() + 1) : "0"+(d.getMonth() + 1)) + "-" + (d.getDate() >9 ? d.getDate() : "0"+d.getDate());
    });
    tab3.addItem(2, lang.tMoveChildTask, document.createElement("input"), "checkbox", function() {
        tab3.arrItems[1].control.focus();
    });
    tab3.addItem(3, lang.buttonOk, document.createElement("input"), "button",
            function() {
                var isMoveChild = tab3.arrItems[1].control.checked;
                var arr = tab3.arrItems[0].control.value.split("-");
                var startDate = (arr.length < 3) ? null : (new Date(arr[0], parseInt(arr[1]) - 1, arr[2]));
                try {
    ...
    var tab4 = this.createTab(4,4, lang.labSet+lang.tDuration, "t", true, this);
    tab4.addItem(1, lang.tDuration, document.createElement("input"), "text", function() {
        tab4.arrItems[0].control.focus();
    });
    tab4.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
    ...
    var tab5 = this.createTab(5,5, lang.labSet+lang.tPercent, "t", true, this);
    tab5.addItem(1, lang.tPercent, document.createElement("input"), "text", function() {
        tab5.arrItems[0].control.focus();
    });
    tab5.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
    ...
    var tab13 = this.createTab(6,13, lang.labSet+lang.tPredecessorTask, "t", true, this);
    tab13.addItem(1, lang.tPredecessorTask, document.createElement("input"), "text", function() {
        tab13.arrItems[0].control.focus();
    });
    tab13.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
    ...
    var tab6 = this.createTab(1,6, lang.pRename, "p", true, this);
    tab6.addItem(1, lang.newName, document.createElement("input"), "text", function() {
        tab6.arrItems[0].control.focus();
    });
    tab6.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
    ...
    var tab7 = this.createTab(2,7, lang.pDelete, "p", true, this);
    tab7.addItem(1, lang.buttonOk, document.createElement("input"), "button",
            function() {
                try {
    ...
    var tab8 = this.createTab(3,8, lang.labSet+lang.pPercent, "p", true, this);
    tab8.addItem(1, lang.pPercent, document.createElement("input"), "text", function() {
        tab8.arrItems[0].control.focus();
    });
    tab8.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
    ...
    tab9.addItem(2, lang.name, document.createElement("input"), "text", function() {
        tab9.arrItems[1].control.focus();
    });
    tab9.addItem(3, lang.tStartDate, document.createElement("input"), "calendar", function() {
        tab9.arrItems[2].control.focus();
    });
    tab9.addItem(4, lang.tDuration, document.createElement("input"), "text", function() {
        tab9.arrItems[3].control.focus();
    });
    tab9.addItem(5, lang.tPercent, document.createElement("input"), "text", function() {
        tab9.arrItems[4].control.focus();
    });
    tab9.addItem(6, lang.tParentTask+"Id", document.createElement("input"), "text", function() {
        tab9.arrItems[5].control.focus();
    });
    tab9.addItem(7, lang.tPredecessorTask+"Id", document.createElement("input"), "text", function() {
        tab9.arrItems[6].control.focus();
    });

    tab9.addItem(9, lang.buttonOk, document.createElement("input"), "button",
            function() {
                try {
                    //var id = tab9.arrItems[0].control.value;
					var maxId = parseInt(self.Chart.maxId) + 1 ;
					var id = maxId;
    ...
    var tab11 = this.createTab(7,11, lang.labAdd+lang.tSuccessorTask, "t", true, this);
    tab11.addItem(1, "Id", document.createElement("input"), "text", function() {
        tab11.arrItems[0].control.focus();
    });
    tab11.addItem(2, lang.name, document.createElement("input"), "text", function() {
        tab11.arrItems[1].control.focus();
    });
    tab11.addItem(3, lang.tStartDate, document.createElement("input"), "calendar", function() {
        tab11.arrItems[2].control.focus();
    });
    tab11.addItem(4, lang.tDuration, document.createElement("input"), "text", function() {
        tab11.arrItems[3].control.focus();
    });
    tab11.addItem(5, lang.tPercent, document.createElement("input"), "text", function() {
        tab11.arrItems[4].control.focus();
    });
    tab11.addItem(6, lang.buttonOk, document.createElement("input"), "button",
            function() {
                try {
    ...
    var tab10 = this.createTab(8,10, lang.labAdd+lang.tChildTask, "t", true, this);
    tab10.addItem(1, "Id", document.createElement("input"), "text", function() {
        tab10.arrItems[0].control.focus();
    });
    tab10.addItem(2, lang.name, document.createElement("input"), "text", function() {
        tab10.arrItems[1].control.focus();
    });
    tab10.addItem(3, lang.tStartDate, document.createElement("input"), "calendar", function() {
        tab10.arrItems[2].control.focus();
    });
    tab10.addItem(4, lang.tDuration, document.createElement("input"), "text", function() {
        tab10.arrItems[3].control.focus();
    });
    tab10.addItem(5, lang.tPercent, document.createElement("input"), "text", function() {
        tab10.arrItems[4].control.focus();
    });
    tab10.addItem(6, lang.buttonOk, document.createElement("input"), "button",
            function() {
                try {
    ...
    var tab12 = this.createTab(5,12,lang.pNewPro, "p", false, this);
    tab12.addItem(1, "Id", document.createElement("input"), "text", function() {
        tab12.arrItems[0].control.focus();
    });
    tab12.addItem(2, lang.name, document.createElement("input"), "text", function() {
        tab12.arrItems[1].control.focus();
    });
    tab12.addItem(3, lang.pStartDate, document.createElement("input"), "calendar", function() {
        tab12.arrItems[2].control.focus();
    });
    tab12.addItem(4, lang.buttonOk, document.createElement("input"), "button",
            function() {
                try {
                    ...
                    var startDatePr = (arr.length < 3) ? null : (new Date(arr[0], parseInt(arr[1]) - 1, arr[2]));
    ...
    tab20.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
                var d = tab20.arrItems[0].control.value;
                try {
                    if (tab20.object.setTaskplanH(d)) tab20.hide();
                } catch(e) {
    ...
    var tab21 = this.createTab(6,21, lang.labSet+lang.pPlanH, "p", true, this);
    tab21.addItem(1, lang.pPlanH, document.createElement("input"), "text", function() {
        tab21.arrItems[0].control.focus();
    });
    tab21.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
                var d = tab21.arrItems[0].control.value;
                try {
						tab21.object.Project.planH=d;
						tab21.object.shiftProjectItem();
						tab21.hide();
                } catch(e) {
    ...
    var tab22 = this.createTab(10,22, lang.labSet+lang.tEndDate, "t", true, this);
    tab22.addItem(1, lang.tEndDate, document.createElement("input"), "calendar", function() {
        tab22.arrItems[0].control.focus();
		var d = tab22.object.getFinishDate();
		this.value=d.getFullYear() + "-" + ((d.getMonth() + 1) > 9 ? (d.getMonth() + 1) : "0"+(d.getMonth() + 1)) + "-" + (d.getDate() >9 ? d.getDate() : ("0" + d.getDate()));
    });
    tab22.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
                var d = tab22.arrItems[0].control.value;
				var arr=d.split("-");
				var ed = new Date(parseInt(arr[0]),(parseInt(arr[1]) - 1),(parseInt(arr[2]) + 1));
				var sd = tab22.object.TaskInfo.StartDate;
				var dur = (ed - sd) / (24 * 60 * 60 * 1000) * tab22.object.Chart.hoursInDay;
                try {
                    if (tab22.object.setDuration(dur)) tab22.hide();
                } catch(e) {
                }
            }
            );
    ...
    var tab23 = this.createTab(9,20, lang.labSet+lang.tAlarm, "t", true, this);
    tab23.addItem(1, lang.tAlarm, document.createElement("input"), "text", function() {
        tab23.arrItems[0].control.focus();
    });
    tab23.addItem(2, lang.buttonOk, document.createElement("input"), "button",
            function() {
                var d = tab23.arrItems[0].control.value;
                try {
                    if (tab23.object.setTaskAlarm(d)) tab23.hide();
                } catch(e) {
...
contextMenu.prototype.showContextMenu = function(x, y, object)
{
    ...
    {
		var proCode=object.Chart.menuProjectCode;

        for (var i = 0; i < this.arrTabs.length; i++) {
            if (this.arrTabs[i].type == "p")
            {
                this.arrTabs[i].object = object;
				if (proCode.substr(this.arrTabs[i].code-1,1)=="1")
				{
				    this.isShow = true;
					this.addItemMenuPanel(this.arrTabs[i]);
					j++;
	...
	if (this.isShow)
	{
		...
		var top=y + this.Chart.heightTaskItem - this.Chart.oData.scrollTop + 20;
		if (top>300)
		{
			top = top - j * 18;
		}
		...
		//this.MenuPanel.style.top = y + this.Chart.heightTaskItem - this.Chart.oData.scrollTop + 5 + "px";
...
contextMenu.prototype.createTab = function(code,id, desc, type, showOInfo, menu)
{
    var tab = new contextMenuTab(code,id, desc, type, showOInfo, menu);
...
function contextMenuTab(code,id, description, type, showOInfo, contextMenu)
{
	this.code=code;
    this.Id = id;
    this.arrItems = [];
...
contextMenuTab.prototype.addItem = function(id, name, control, type, handler)
{
	...
	{
		type="text";
		//control.readOnly=true; //在这里添加日历
		//control.onfocus="setday(this)";
		control.id="calendarid";
		//control.onclick="alert('b')";
		//control.id="bb"; 
	}
    control.type = type;
...
{
	var lang = this.contextMenu.Chart.ChartLang;
    ...
    {
        if (this.object) {
			var isIE=this.object.Chart._isIE;
            if (this.object.constructor == GanttTask) {
                this.insertData(t, "ID", this.object.TaskInfo.Id);
                this.insertData(t, lang.name, this.object.TaskInfo.Name);
                this.insertData(t, lang.tDuration, this.object.TaskInfo.Duration);
                this.insertData(t, lang.tPlanH, this.object.TaskInfo.planH);
                this.insertData(t, lang.tAlarm, this.object.TaskInfo.alarm);

                this.insertData(t, lang.tPercent, this.object.TaskInfo.PercentCompleted);
                this.insertData(t, lang.tStartDate, this.object.TaskInfo.StartDate.getFullYear() + "-" + (this.object.TaskInfo.StartDate.getMonth() + 1) + "-" + this.object.TaskInfo.StartDate.getDate());

				var ed =  this.object.getFinishDate();
                this.insertData(t, lang.tEndDate, ed.getFullYear() + "-" + (ed.getMonth() + 1) + "-" + ed.getDate());


				var preTask=this.object.Project.getTaskById(this.object.TaskInfo.PredecessorTaskId);
				var preTaskName="";
				if (preTask)
				{
					preTaskName=preTask.TaskInfo.Name;
				}
                this.insertData(t, lang.tPredecessorTask, this.object.TaskInfo.PredecessorTaskId+preTaskName);
            } else
            {
                this.insertData(t, "Idú", this.object.Project.Id);
                this.insertData(t,lang.name, this.object.Project.Name);
                this.insertData(t,lang.pStartDate, this.object.Project.StartDate.getFullYear() + "-" + (this.object.Project.StartDate.getMonth() + 1) + "-" + this.object.Project.StartDate.getDate());
                this.insertData(t, lang.pPlanH, this.object.Project.planH);
            }
        }
    }
    ...
    for (var i = 0; i < this.arrItems.length; i++) {
        ...
        } else
        {
			...
			this.arrItems[i].control.value = "";
            c2.appendChild(this.arrItems[i].control);
			if (this.arrItems[i].Name == "Id")
			{
...
*/
	if (!isIE)
	{
		...
		var bnt=panel.querySelectorAll("input[type=button]");
		for (var i=0; i<bnt.length; i++)
		{
			bnt[i].className="button";
		}
	}
...
function bgEvent(div,obj)
{
	if (obj.isTask)
	{
		var id = obj.TaskInfo.Id;
		var t = "bg_";
	}
	if (obj.isProject)
	{
	...
	var div_mouseover = function(e){
		var div_bg = document.getElementById(t + id);
		div_bg.onmouseover();
	};
	obj.addEvent(div, 'mouseover', div_mouseover, false);

	var div_mouseout = function(e){
		var div_bg = document.getElementById(t + id);
		div_bg.onmouseout();
	};
	obj.addEvent(div, 'mouseout', div_mouseout, false);

	var div_click = function(e){
		var div_bg = document.getElementById(t + id);
		div_bg.onclick();
	};
	obj.addEvent(div, 'click', div_click, false);
}
