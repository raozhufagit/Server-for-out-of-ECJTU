...
public partial class ScManufact_ScDetail : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            if (!LCX.Common.PublicMethod.StrIFIn("," + userName + ",", "," + scSet.sc_SUser + ","))
            {
    ...
    public void DataBindToGridview()
    {
        ...
        {
            //订单审核权利的可以查看所有
            GVData.DataSource = MyModel.GetList("Pc_State<5 and Pc_State>1 and Pc_Name Like '%" + this.TextBox1.Text + "%' order by ID desc");
        }
        else
        {
            LCX.Common.MessageBox.ShowAndRedirect(this, "当前没有权限！", ViewState["UrlReferrer"].ToString());
    ...
    protected void GVData_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            switch (Number)
            {
                ...
                case 2: { e.Row.Cells[5].Text = "<a class=\"bbk button small-long-btn\" href=\"ScDetail22.aspx?ID=" + DataBinder.Eval(e.Row.DataItem, "ID").ToString() + "\">审核进度</a>"; break; } //技术审核通过
