(function($){
	...
	$.addGrid = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 errormsg: td_lang.inc.msg_42,//'网络连接错误'
			 usepager: false, //
			 nowrap: true, //
			 page: 1, //current page
			 total: 1, //total pages
			 ...
			 rp_cookie: '', //每页显示数量cookie
			 rpOptions: [10,15,20,25,40],
			 title: false,
			 pagestat: '第 {from} 至 {to} 条， 共 {total} 条',
			 procmsg: td_lang.inc.msg_43,//'加载中，请稍后 ...'
			 colmsg: td_lang.inc.msg_44,//'显示/隐藏列'
			 query: '',
			 qtype: '',
			 nomsg: td_lang.inc.msg_45,//'无数据'
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				$('.pReload',this.pDiv).removeClass('pLoading');
				this.loading = false;
			...
			populate: function () { //get latest data
				...
				$('.pReload',this.pDiv).addClass('pLoading');
						...
						function ()
							{
									...
									if (pth!=null)
									{
                                     ...
                                     //$(tdDiv).css({height:tdHeight, "line-height" :tdHeight});     
			...
			addHeader:function()
			{
			    var thead = g.parseHeader();
			    $(t).prepend(thead);
			},
			updateHeader:function()
			{
			    var thead = g.parseHeader();
			    $("table",g.hDiv).empty().html(thead);
			    g.setupHeader();
			},
		...
		$(t).before(g.bDiv);
		$(g.bDiv)
		.css({ height: (p.height=='auto') ? 'auto' : (p.height-30-25-6)+"px"})
		...
		if (p.usepager)
		{
		    ...
		    var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">第 <input type="text" size="2" value="1" /> 页，共 <span>  </span> 页</span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
			...
			if (p.useRp)
			{
			    ...
			    */
			    //可输入每页显示数量
			    var msg1 = sprintf(td_lang.inc.msg_120,"<input type='text' name='rp' value='"+p.rp+"' size=3>");
			    $('.pDiv2',g.pDiv).prepend("<div class='pGroup'><span class='pcount'>"+msg1+"</span></div> <div class='btnseparator'></div>");
			    $('input[name="rp"]',g.pDiv).bind("keydown blur",
			    		function ()
			    		{
			    			if (p.onRpChange) 
			    			...
			    			else
			    			{
			    			    if(event.keyCode==13)
				                {
			    			        var perPage = parseInt(this.value);
	  			                    if(perPage>99)
	  			                    {
	  				                    var msg=td_lang.inc.msg_46;//'此操作会导致比较缓慢,确定要执行吗?'
	                                    if(!window.confirm(msg))
	           	                            return;
	                                }
	                                else if(perPage==0 || perPage%1!=0)
	                                {
	                                	alert(td_lang.inc.msg_47);//"请输入有效数字!"
	                                	return;
	                                }
	                                //存入cookie
	                                if(p.rp_cookie)
	                                {
	                                    ...
	                                    document.cookie = p.rp_cookie+"="+ perPage + ";expires=" + exp.toGMTString()+";path=/";
	                                }
			    				    p.newp = 1;
			    				    p.rp = perPage;
			    				    g.populate();
			...
			$.each(p.colModel,
			 	function (i,row)
				{
				    if(row.checkable!=false)
				    {
					    ...
					    $('tbody',g.nDiv).append('<tr><td class="ndcol1"><input type="checkbox" '+ chk +' class="togCol" value="'+ cn +'" /></td><td class="ndcol2">'+row.display+'</td></tr>');
					    cn++;
				    }
				}
			);
	...
	$.fn.gridReload = function(p) { // function to reload grid
		...
		return this.each( function() {
				if (this.grid&&this.p.url) 
				{
				    ...
				    if(p.colModel)
				    {
				        //alert( $(this.grid.hDiv).html());
				        this.grid.updateHeader();
				    }
				    this.grid.populate();
				}
			});
