...
function showContextMenu(event,type)
{
  ...
  case 1:
  	...
  	sMenu += getMenuRow("Edit_Process("+Process_ID+")", "节点属性");
  	height += 20;

//  	sMenu += getMenuRow("set_next("+Process_ID+")", "在此步骤下追加");
//  	height += 20;

  	sMenu += getMenuRow("set_item("+Process_ID+")", "可写设置");
  	height += 20;
  	
  	sMenu += getMenuRow("set_secret("+Process_ID+")", "保密设置");
  	height += 20;
...
//  	height += 20;
  	...
  	sMenu += getMenuRow("Del_Process("+Process_ID+")", "删除节点");
  	height += 20;
  ...
  case 2:
  	...
  	sMenu += getMenuRow("Add_Process()", "新建节点");
  	height += 20;
...
{
   var WorkFlowID=document.getElementById("FlowNumber").value;
   
   document.getElementById("SET_SQL").value="";
   var id = 0;
   var strSql = '';
   var mf_pixel_left = 0;
   var mf_pixel_top = 0;
   ...
   for (var i = 0; i < a.length; i++)
   {
      ...
      if (table_id > 0)
      {
         strSql += "update LCX_WorkFlowNode set NodeLeft=" + mf_pixel_left + ",NodeTop=" + mf_pixel_top + " where ID="+table_id+";";
      }
   }
...
var str11=document.getElementById("SET_SQL").value;
LoadWindow("SetNodeMapSave.aspx?WorkFlowID="+WorkFlowID+"&SQLStr="+str11);
}
...
{
var str11=document.getElementById("FlowNumber").value;
LoadWindow("NWorkFlowNodeAdd.aspx?WorkFlowID="+str11+"");
}
...
{
    var str11=document.getElementById("FlowNumber").value;
	LoadWindow("NWorkFlowNodeModify.aspx?WorkFlowID="+str11+"&ID="+Process_ID);
}

function set_next(Process_ID)
{
    var str11=document.getElementById("FlowNumber").value;
	LoadWindow("SetNodeNext.aspx?WorkFlowID="+str11+"&ID="+Process_ID);
}
...
{
    var str11=document.getElementById("FlowNumber").value;
	LoadWindow("SetNodeWrite.aspx?WorkFlowID="+str11+"&ID="+Process_ID);
}
//条件字段
function set_condition(Process_ID)
{
    var str11=document.getElementById("FlowNumber").value;
	LoadWindow("SetNodeCondition.aspx?WorkFlowID="+str11+"&ID="+Process_ID);
}
//保密字段
function set_secret(Process_ID)
{
    var str11=document.getElementById("FlowNumber").value;
	LoadWindow("SetNodeSecret.aspx?WorkFlowID="+str11+"&ID="+Process_ID);
}
...
function Del_Process(Process_ID)
{
   ...
   {   
    var str11=document.getElementById("FlowNumber").value;
	LoadWindow("SetNodeDel.aspx?WorkFlowID="+str11+"&ID="+Process_ID);
	}
}
