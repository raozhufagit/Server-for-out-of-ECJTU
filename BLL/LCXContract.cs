...
namespace LCX.BLL
{
	...
	public class LCXContract
	{
        ...
        public LCXContract(int idstr)
		{
			...
			strSql.Append(" FROM LCXContract ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXContract"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXContract");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXContract(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXContract set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXContract ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update LCXContract ");
            strSql.Append(" set HTZJ=(select sum(b.ZongJia) from LCXCProduct b where b.HeTongName=a.HeTongName),YFFY=(select sum(convert(float, c.HTJE)) from LCXYS c where c.HeTongName=a.HeTongName and c.SFDK='是' and c.NowState='S'),SQFY= HTZJ -YFFY , SFJF=(select case when exists(select * from LCXCProduct b where b.HeTongName=a.HeTongName and b.IFJiaoFu='未交付') then '未交付' when not exists (select * from LCXCProduct b where b.HeTongName=a.HeTongName) then '未交付' else '已交付' end),NowState= (select case  when  exists (select * from LCXBuyOrder F where F.OrderName=a.HeTongName and SFJF='已交付') and NowState like '%通过审核%'  then '完成采购' else NowState end) from LCXContract a");
        ...
        public string GetSerils()
        {
            ...
            string sSql = "select HeTongSerils from LCXContract where HeTongSerils like @PreFix order by ID desc ";
            SqlParameter[] parameters = {
