...
namespace LCX.BLL
{
	...
	public class LCXCarLog
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXCarLog"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXCarLog");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXCarLog(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXCarLog set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXCarLog ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
