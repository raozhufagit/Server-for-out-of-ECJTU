...
namespace LCX.BLL
{
	...
	/// 类LCXCustomFuWu。
	/// </summary>
	public class LCXCustomFuWu
	{
		public LCXCustomFuWu()
		{}
		#region Model
		private int _id;
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXCustomFuWu(int idstr)
		{
			...
			strSql.Append(" FROM LCXCustomFuWu ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXCustomFuWu"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXCustomFuWu");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXCustomFuWu(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXCustomFuWu set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete LCXCustomFuWu ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
