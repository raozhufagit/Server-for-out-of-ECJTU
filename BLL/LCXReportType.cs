...
namespace LCX.BLL
{
	...
	/// 类LCXReportType。
	/// </summary>
	public class LCXReportType
	{
		public LCXReportType()
		{}
		#region Model
		private int _id;
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXReportType(int idstr)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,TypeName,BackInfo,PaiXuStr ");
			strSql.Append(" FROM LCXReportType ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXReportType"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXReportType");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXReportType(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXReportType set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXReportType ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
