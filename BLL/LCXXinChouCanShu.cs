...
namespace LCX.BLL
{
	...
	public class LCXPayCanShu
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXPayCanShu"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXPayCanShu");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXPayCanShu(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXPayCanShu set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXPayCanShu ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
