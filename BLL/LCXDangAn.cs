...
namespace LCX.BLL
{
	...
	public class LCXDangAn
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXDangAn"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXDangAn");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXDangAn(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXDangAn set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXDangAn ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
