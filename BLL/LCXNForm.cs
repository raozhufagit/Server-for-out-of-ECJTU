...
namespace LCX.BLL
{
	...
	public class LCX_Form
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCX_Form"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCX_Form");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCX_Form(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCX_Form set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCX_Form ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
