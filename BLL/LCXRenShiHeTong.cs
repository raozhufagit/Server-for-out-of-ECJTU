...
namespace LCX.BLL
{
	...
	/// 类LCXP_Contract。
	/// </summary>
	public class LCXP_Contract
	{
		public LCXP_Contract()
		{}
		#region Model
		private int _id;
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXP_Contract(int idstr)
		{
			...
			strSql.Append(" FROM LCXP_Contract ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXP_Contract"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXP_Contract");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXP_Contract(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXP_Contract set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXP_Contract ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
