...
namespace LCX.BLL
{
	...
	public class LCXSupplys
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXSupplys"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXSupplys");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXSupplys(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXSupplys set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXSupplys ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
