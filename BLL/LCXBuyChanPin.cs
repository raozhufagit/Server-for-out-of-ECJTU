...
namespace LCX.BLL
{
	...
	/// 类LCXBuyChanPin。
	/// </summary>
	public class LCXBuyChanPin
	{
		public LCXBuyChanPin()
		{}
		#region Model
		private int _id;
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXBuyChanPin(int idstr)
		{
			...
			strSql.Append(" FROM LCXBuyChanPin ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXBuyChanPin"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXBuyChanPin");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXBuyChanPin(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXBuyChanPin set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXBuyChanPin ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
