...
namespace LCX.BLL
{
	...
	public class LCXBaoJia
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXBaoJia"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXBaoJia");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXBaoJia(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXBaoJia set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete LCXBaoJia ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
