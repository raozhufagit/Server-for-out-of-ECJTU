...
namespace LCX.BLL
{
	...
	public class LCXJiXiao
	{
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXJiXiao(int idstr)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,JXName,JianJie,UserName,TimeStr ");
			strSql.Append(" FROM LCXJiXiao ");
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				JXName=ds.Tables[0].Rows[0]["JXName"].ToString();
		...
		return DbHelperSQL.GetMaxID("ID", "LCXJiXiao"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXJiXiao");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXJiXiao(");
			strSql.Append("JXName,JianJie,UserName,TimeStr)");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXJiXiao set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXJiXiao ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,JXName,JianJie,UserName,TimeStr ");
			strSql.Append(" FROM LCXJiXiao ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
