...
namespace LCX.BLL
{
	...
	public class LCXCarBY
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXCarBY"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXCarBY");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXCarBY(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXCarBY set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXCarBY ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
