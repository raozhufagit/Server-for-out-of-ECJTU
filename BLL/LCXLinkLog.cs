...
namespace LCX.BLL
{
	...
	public class LCXLinkLog
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXLinkLog"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXLinkLog");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXLinkLog(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXLinkLog set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete LCXLinkLog ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
