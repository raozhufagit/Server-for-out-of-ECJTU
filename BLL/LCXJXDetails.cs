...
namespace LCX.BLL
{
	...
	public class LCXJXDetails
	{
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXJXDetails(int idstr)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,JXID,ItemsID,UserName,NumStr ");
			strSql.Append(" FROM LCXJXDetails ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXJXDetails"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXJXDetails");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXJXDetails(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXJXDetails set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXJXDetails ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
