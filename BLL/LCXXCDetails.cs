...
namespace LCX.BLL
{
	...
	public class LCXXCDetails
	{
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXXCDetails(int idstr)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,XCID,ItemsID,UserName,NumStr ");
			strSql.Append(" FROM LCXXCDetails ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXXCDetails"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXXCDetails");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXXCDetails(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXXCDetails set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXXCDetails ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
