...
namespace LCX.BLL
{
	...
	/// 类LCXCustomNeed。
	/// </summary>
	public class LCXCustomNeed
	{
		public LCXCustomNeed()
		{}
		#region Model
		private int _id;
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXCustomNeed(int idstr)
		{
			...
			strSql.Append(" FROM LCXCustomNeed ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXCustomNeed"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXCustomNeed");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXCustomNeed(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXCustomNeed set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete LCXCustomNeed ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
