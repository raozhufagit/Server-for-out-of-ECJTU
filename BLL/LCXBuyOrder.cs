...
namespace LCX.BLL
{
    ...
    public class LCXBuyOrder
    {
        ...
        public LCXBuyOrder(int idstr)
        {
            ...
            strSql.Append(" FROM LCXBuyOrder ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from LCXBuyOrder");
            strSql.Append(" where ID=@ID ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into LCXBuyOrder(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update LCXBuyOrder set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from LCXBuyOrder ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update LCXBuyOrder ");
            strSql.Append(" set HTZJ=(select sum(b.ZongJia) from LCXBuyChanPin b where b.OrderName=a.OrderName),YFFY=(select sum(convert(float, c.HTJE)) from LCXYS c where c.HeTongName=a.OrderName and c.SFDK='是' and c.NowState='F'),SQFY= HTZJ -YFFY , SFJF=(select case when exists(select * from LCXBuyChanPin b where b.OrderName=a.OrderName and b.IFJiaoFu='未交付') then '未交付' when not exists (select * from LCXBuyChanPin b where b.OrderName=a.OrderName) then '未交付' else '已交付' end) from LCXBuyOrder a");
        ...
        { 
            ...
            string sSql = "select Serils from LCXBuyOrder where Serils like @PreFix order by ID desc ";
            SqlParameter[] parameters = {
