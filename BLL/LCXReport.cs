...
namespace LCX.BLL
{
	...
	public class LCXReport
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXReport"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXReport");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXReport(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXReport set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXReport ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
