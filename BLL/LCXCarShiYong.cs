...
namespace LCX.BLL
{
	...
	public class LCXCarUse
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXCarUse"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXCarUse");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXCarUse(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXCarUse set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXCarUse ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
