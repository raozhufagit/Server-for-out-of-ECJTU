...
namespace LCX.BLL
{
	...
	/// 类LCX_WorkDetails。
	/// </summary>
	public class LCX_WorkDetails
	{
		public LCX_WorkDetails()
		{}
		#region Model
		private int _id;
		private int? _workid;
		...
		return DbHelperSQL.GetMaxID("ID", "LCX_WorkDetails"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCX_WorkDetails");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCX_WorkDetails(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCX_WorkDetails set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCX_WorkDetails ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
