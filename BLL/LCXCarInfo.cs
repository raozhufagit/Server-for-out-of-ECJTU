...
namespace LCX.BLL
{
	...
	public class LCXCarInfo
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXCarInfo"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXCarInfo");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXCarInfo(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXCarInfo set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXCarInfo ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
