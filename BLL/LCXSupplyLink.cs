...
namespace LCX.BLL
{
	...
	/// 类LCXSupplyLink。
	/// </summary>
	public class LCXSupplyLink
	{
		public LCXSupplyLink()
		{}
		#region Model
		private int _id;
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXSupplyLink(int idstr)
		{
			...
			strSql.Append(" FROM LCXSupplyLink ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXSupplyLink"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXSupplyLink");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXSupplyLink(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXSupplyLink set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXSupplyLink ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
