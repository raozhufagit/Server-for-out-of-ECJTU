...
namespace LCX.BLL
{
	...
	public class LCXTrain
	{
		...
		return DbHelperSQL.GetMaxID("ID", "LCXTrain"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXTrain");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXTrain(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXTrain set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXTrain ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
