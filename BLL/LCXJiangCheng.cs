...
namespace LCX.BLL
{
	...
	/// 类LCXPunishment。
	/// </summary>
	public class LCXPunishment
	{
		public LCXPunishment()
		{}
		#region Model
		private int _id;
		...
		/// 得到一个对象实体
		/// </summary>
        public LCXPunishment(int idstr)
		{
			...
			strSql.Append(" FROM LCXPunishment ");
		...
		return DbHelperSQL.GetMaxID("ID", "LCXPunishment"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from LCXPunishment");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into LCXPunishment(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update LCXPunishment set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from LCXPunishment ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = idstr;
